    ...
    def _build_ui(self):
        with SinglePageWithDrawerLayout(self.server, full_height=True) as self.ui:
            ...
            with self.ui.content:
                with dataclass.Provider(name="item", instance=("selected?.[0]",)):
                    with v3.VCard(v_if="item_available && item._id", classes="ma-4"):
                        with v3.VContainer():
                            with v3.VRow():
                                with v3.VCol():
                                    v3.VTextField(
                                        v_model="item.first_name",
                                        label="First Name",
                                    )
                                with v3.VCol():
                                    v3.VTextField(
                                        v_model="item.last_name",
                                        label="Last Name",
                                    )
